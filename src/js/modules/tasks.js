...
function renderTasksList() {
  ...
  if (doneTasks.length > 0) {
    tableHTML += `
      ...
      ${doneTasks.map(task => renderTaskRow(task, true)).join('')}
    `;
  }
...
function renderTaskRow(task, isCompletedDrawer = false) {
  ...
  return `
    <tr class="${isCompletedDrawer ? 'completed-task-row' : ''}" style="${isCompletedDrawer ? 'display: none; ' : ''}${isDone ? 'opacity: 0.5;' : ''}">
      ...
      <td style="text-align: right;">
        <div style="display: flex; justify-content: flex-end; align-items: center; gap: 8px;">
          <button class="btn btn-icon btn-cycle" data-id="${task.id}" style="background: transparent; border: none; opacity: 0.6; padding: 0.2rem; display: flex; align-items: center; justify-content: center;" title="Cycle status">
          ...
          <button class="btn btn-icon btn-edit" data-id="${task.id}" style="background: transparent; border: none; opacity: 0.6; padding: 0.2rem; display: flex; align-items: center; justify-content: center;" title="Modify Task">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
          </button>
          <button class="btn btn-icon btn-delete" data-id="${task.id}" style="background: transparent; border: none; color: var(--danger); opacity: 0.6; padding: 0.2rem; display: flex; align-items: center; justify-content: center;" title="Purge Task">
